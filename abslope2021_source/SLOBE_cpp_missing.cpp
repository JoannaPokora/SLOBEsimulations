...
void gamma_mean_update(const NumericVector & abs_beta_ord, const NumericVector & lambda, const int & p, NumericVector & gamma_h) {
...
// [[Rcpp::export]]
void Center_and_scale(mat& X,const int& n, const int& p){
  for (int i=0; i<p; ++i){
...
// [[Rcpp::export]]
List SLOBE_ADMM_approx_missing(NumericVector start, mat Xmis, NumericMatrix Xinit, vec Y, double a_prior, double b_prior,mat Covmat, double sigma = 1.0, 
				double FDR = 0.05, double tol = 1e-04, bool known_sigma = false, int max_iter=100, bool verbose = false,bool BH=true, bool known_cov=false) {
  ...
  double error = 0.0;
    ...
    if(!known_cov){
		...
		impute_advance(beta_new, X, Y, S, sigma_sq, n, p, mu, XisFin, anyNanXrows, nanIndicesInRow);
		Center_and_scale(X,n,p); 
	...
	return List::create(Named("beta")=beta, Named("sigma")=sigma, Named("theta")=theta, Named("c")=c,
                           Named("w")=w, Named("converged")=converged, Named("X")=X,Named("Sigma")=Sigma,Named("mu")=mu,Named("lambda")=lambda);
}
